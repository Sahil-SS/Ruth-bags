...
const MobileNavbar = () => {
  ...
  const navItems = [
    ...
    { label: 'WhatsApp', icon: <WhatsAppIcon size={20} />, href: whatsappUrl, isExternal: true, isWhatsApp: true },
  ];
          ...
          const NavContent = (
            <div className="flex flex-col items-center justify-center py-1">
              <span className={`transition-all duration-300 ${
                item.isWhatsApp
                  ? 'text-[#25D366]'  // Always green for WhatsApp
                  : isActive ? 'text-black scale-110' : 'text-gray-400'
              }`}>
                {item.icon}
              </span>
              <span className={`text-[9px] uppercase tracking-[0.15em] mt-1.5 font-serif italic transition-colors ${
                item.isWhatsApp
                  ? 'text-[#25D366]'
